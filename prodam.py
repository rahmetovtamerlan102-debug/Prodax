...
<body>
    ...
    <script>
        const LEVELS = {
            ...
            medium: { name: "Средний", icon: "🟡", price: 249, desc: "Расширенный функционал, доп. модули", features: ["Дополнительные модули", "Гибкая настройка", "Приоритетная поддержка"] },
            hard: { name: "Сложный", icon: "🔴", price: 499, desc: "Полный функционал, ML модель", features: ["ML модель", "Полная админ-панель", "API доступ"] }
        };
        ...
        function renderShop() {
            ...
            document.getElementById('shop-view').innerHTML = Object.entries(LEVELS).map(([k, l]) => `
                <div class="level-card ${selectedLevel === k ? 'selected' : ''}" onclick="selectLevel('${k}')">
                    <div class="level-header">
                        <div class="level-name">${l.icon} ${l.name}</div>
                        <div class="level-price">${l.price}₽</div>
                    </div>
                    <div class="level-desc">${l.desc}</div>
                    <div class="features">${l.features.map(f => `<span class="feature-tag">✓ ${f}</span>`).join('')}</div>
                </div>
            `).join('') + `<button class="btn btn-primary" ${!selectedLevel ? 'disabled' : ''} onclick="showPayment()">💎 Заказать бота</button>`;
        }
            ...
            document.getElementById('shop-view').innerHTML = `
                <div class="payment-panel">
                    ...
                    <div class="payment-details">
                        <div class="payment-row"><span>🤖 Бот:</span><span>${l.name}</span></div>
                        <div class="payment-row"><span>🏦 Банк:</span><span>${BANK_NAME}</span></div>
                        <div class="card-number" onclick="copyCard()">${CARD_NUMBER}</div>
                        <div class="payment-row"><span>👤 Получатель:</span><span>${RECIPIENT_NAME}</span></div>
                    </div>
                    ...
                    <button class="btn btn-secondary" style="margin-top:10px;" onclick="goBack()">◀ Назад</button>
                </div>
            `;
        }
            ...
            try {
                ...
                document.getElementById('profile-view').innerHTML = `
                    <div class="profile-card">
                        ...
                        <div class="user-name">${tgUser?.first_name || 'Пользователь'}</div>
                        <div class="user-id">ID: ${tgUser?.id || 'Неизвестно'}</div>
                        <div class="order-history">
                            <h3 style="margin-bottom:15px;">📋 История заказов</h3>
                            ${data.orders?.length ? data.orders.map(o => `
                                <div class="order-item">
                                    <div>
                                        <span>${o.complexity === 'easy' ? '🟢' : o.complexity === 'medium' ? '🟡' : '🔴'} ${o.complexity === 'easy' ? 'Лёгкий' : o.complexity === 'medium' ? 'Средний' : 'Сложный'}</span>
                                        <div style="font-size:12px; color:#888;">${o.price}₽ • ${o.created_at?.slice(0,16)}</div>
                                    </div>
                                    <span class="status-badge status-${o.status}">${o.status === 'completed' ? '✅ Выполнен' : o.status === 'pending' ? '⏳ На проверке' : '❌ Отклонён'}</span>
                                </div>
                            `).join('') : '<div style="text-align:center; padding:20px;">📭 У вас пока нет заказов</div>'}
                        </div>
                        ${hasCompleted ? '<button class="btn btn-primary" style="margin-top:15px;" onclick="openReviewModal()">✍️ Оставить отзыв</button>' : ''}
                    </div>
                `;
            } catch (error) {
                document.getElementById('profile-view').innerHTML = '<div class="loading">❌ Ошибка загрузки</div>';
            }
        }
        ...
        async function loadReviews() {
            ...
            try {
                ...
                document.getElementById('reviews-view').innerHTML = `
                    <h3 style="margin-bottom:15px;">⭐ Отзывы покупателей</h3>
                    ${data.reviews?.length ? data.reviews.map(r => `
                        <div class="review-item">
                            <div style="display:flex; justify-content:space-between;">
                                <span>👤 ${r.username}</span>
                                <span>${r.complexity === 'easy' ? '🟢' : r.complexity === 'medium' ? '🟡' : '🔴'}</span>
                            </div>
                            <div class="review-text">${r.review}</div>
                            <div style="font-size:10px; color:#666; margin-top:8px;">${r.created_at?.slice(0,10)}</div>
                        </div>
                    `).join('') : '<div class="loading">⭐ Пока нет отзывов. Будьте первым!</div>'}
                `;
            } catch (error) {
                document.getElementById('reviews-view').innerHTML = '<div class="loading">❌ Ошибка загрузки</div>';
            }
        }
...
def create_order():
    try:
        ...
        bot.send_message(
            ...
            f"{level_icon} {complexity.upper()} — {price}₽\n"
...
def get_orders():
    ...
    return jsonify({"orders": [{"id": o[0], "complexity": o[3], "price": o[4], "status": o[6], "created_at": o[7]} for o in orders]})

@flask_app.route('/get_reviews', methods=['GET'])
def get_reviews_endpoint():
    return jsonify({"reviews": [{"id": r[0], "username": r[2], "complexity": r[3], "review": r[4], "created_at": r[5]} for r in get_reviews()]})
...
def admin_view_order(call):
    ...
    text += f"{level_icon} Уровень: {order[3].upper()}\n"
    text += f"💰 Сумма: {order[4]}₽\n"
    text += f"👤 Пользователь: @{order[2] or order[1]}\n"
    text += f"📅 Создан: {order[7][:16]}\n"
    text += f"📌 Статус: {status_text}\n"
    
    if order[5]:
        text += f"\n📸 *Скриншот оплаты:*"
        bot.send_photo(call.message.chat.id, order[5], caption=text, parse_mode="Markdown", reply_markup=order_action_keyboard(order_id))
    else:
...
def admin_confirm_order(call):
    ...
    if order:
        ...
        bot.send_message(
            ...
            f"💰 Сумма: {order[4]}₽\n\n"
            f"Спасибо за покупку! Вы можете оставить отзыв в профиле.",
            parse_mode="Markdown"
        )
