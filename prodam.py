...
LEVELS = {
    ...
    "medium": {
        ...
        "features": [
            "Всё из Лёгкого",
            "Ручная рассылка по пользователям",
            "Админ-панель в боте",
            "Чёрный список слов",
...
def start(m):
    ...
    bot.send_message(m.chat.id, "🤖 BotShop PRO MAX\n👇 Нажми на кнопку", reply_markup=kb)
    ...
    if target == "all":
        ...
        for user in users:
            try:
                ...
                pass
        bot.send_message(admin_chat_id, f"✅ Рассылка отправлена {len(users)} пользователям")
    else:
        c = conn()
        ...
        for user in users:
            try:
                ...
                pass
        bot.send_message(admin_chat_id, f"✅ Рассылка отправлена {len(users)} покупателям")
...
.features{margin-top:10px;font-size:13px;color:#ccc}
.features li{margin-left:20px;padding:3px 0}
...
const LEVELS = {
    easy: { name: "Лёгкий", icon: "🟢", price: 99, features: ["Базовая защита от спама", "Автоматические ответы на команды", "Приветственное сообщение", "Кнопка 'Контакты'", "Логирование до 100 сообщений"] },
    medium: { name: "Средний", icon: "🟡", price: 249, features: ["Всё из Лёгкого", "Ручная рассылка по пользователям", "Админ-панель в боте", "Чёрный список слов", "Статистика активности", "Автоматические ответы на частые вопросы"] },
    hard: { name: "Сложный", icon: "🔴", price: 499, features: ["Всё из Среднего", "Автоматическая рассылка по расписанию", "Интеграция с Google Sheets", "Гибкая система скидок для клиентов", "Полная аналитика продаж", "Поддержка до 1000 пользователей одновременно"] }
};
...
function renderShop() {
    ...
    for (let [k, l] of Object.entries(LEVELS)) {
        html += `<div class="card ${selectedLevel === k ? 'selected' : ''}" onclick="selectLevel('${k}')">
            <h3>${l.icon} ${l.name} — ${l.price}₽</h3>
            <div class="features"><ul>${l.features.map(f => `<li>${f}</li>`).join('')}</ul></div>
        </div>`;
    }
