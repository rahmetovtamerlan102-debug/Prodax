...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .toast { position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.9); padding: 12px 20px; border-radius: 50px; z-index: 2000; animation: fadeOut 2s; }
...
<body>
    ...
    <script>
        ...
        async function confirmOrder() {
            ...
            try {
                const res = await fetch('/create_order', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ complexity: selectedLevel, user_id: tgUser?.id || 0, username: tgUser?.username || tgUser?.first_name || 'user', screenshot: selectedScreenshot }) });
