...
def check_payment_screenshot(image_bytes):
    """Проверяет, похоже ли изображение на скриншот оплаты"""
    reasons = []
    confidence = 0
    payment_found = []
    bank_found = []
    ...
    try:
        ...
        if img is None:
            return False, 0, ["не удалось прочитать изображение"]
        ...
        text = ""
        try:
            text = pytesseract.image_to_string(gray, lang='eng+rus', config='--psm 6', timeout=10)
        except:
            try:
        ...
        # Проверка ключевых слов оплаты
        for kw in PAYMENT_KEYWORDS:
            if kw in text_lower:
                payment_found.append(kw)
                confidence += 8
        
        # Проверка банковских ключевых слов
        for kw in BANK_KEYWORDS:
            if kw in text_lower:
                bank_found.append(kw)
                confidence += 12
        ...
        for pattern in amount_patterns:
            ...
            if match:
                ...
                confidence += 20
                reasons.append(f"найдена сумма {amount_value}")
                break
        ...
        if has_date:
            confidence += 15
            reasons.append(f"найдена дата {date_match.group()}")
        
        # Проверка на QR-код
        try:
            qr_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY) if len(img.shape) == 3 else img
            qr_detector = cv2.QRCodeDetector()
            qr_data, _, _ = qr_detector.detectAndDecode(qr_img)
            if qr_data:
                confidence += 20
                reasons.append("найден QR-код")
        except Exception as e:
            print(f"QR detection error: {e}")
        
        # Усиленная проверка: обязательны сумма + дата + (банк или оплата)
        is_payment = has_amount and has_date and (len(bank_found) >= 1 or len(payment_found) >= 2)
        
        # Если уверенность очень высокая, тоже пропускаем
        if not is_payment and confidence >= 70:
            is_payment = True
        ...
        return False, 0, f"ошибка проверки: {e}"
...
def simple_ocr(image_bytes):
    """Базовое OCR для извлечения данных"""
    try:
        nparr = np.frombuffer(image_bytes, np.uint8)
        img = cv2.imdecode(nparr, cv2.IMREAD_COLOR)
...
# ================= SHOP =================
LEVELS = {
    "easy": {"name": "Лёгкий", "price": 99, "icon": "🟢", "color": "#22c55e", 
             "features": ["Защита от спама", "Автоответы", "Приветствие", "Кнопка меню", "Логи 100 сообщ"]},
    "medium": {"name": "Средний", "price": 249, "icon": "🟡", "color": "#eab308", 
               "features": ["Всё из Лёгкого", "Рассылка", "Админ-панель", "Чёрный список", "Статистика"]},
    "hard": {"name": "Сложный", "price": 499, "icon": "🔴", "color": "#ef4444", 
...
def create_order():
    try:
        ...
        if not is_payment:
            ...
            return jsonify({
                "ok": False, 
                "error": "❌ Загруженное изображение не похоже на чек оплаты. Пожалуйста, загрузите скриншот из приложения банка с суммой и датой."
            })
        
        # OCR для извлечения суммы
...
body {
    ...
    background: radial-gradient(ellipse at 30% 20%, #0a0a2a, #050510);
    color: #fff;
    min-height: 100vh;
    padding: 16px;
}
...
.card-number {
    background: linear-gradient(135deg, #0a0a1a, #0d0d1a);
    padding: 16px;
    border-radius: 20px;
    text-align: center;
...
.card-number:active {
    background: linear-gradient(135deg, #1a1a2a, #1a1a2a);
...
<div id="reviewModal" class="modal">
    <div class="modal-content">
        ...
        <textarea id="reviewText" rows="4" placeholder="Поделитесь впечатлениями о боте..."></textarea>
...
const LEVELS = {
    easy: { name: "Лёгкий", icon: "🟢", price: 99, features: ["Защита от спама", "Автоответы", "Приветствие", "Кнопка меню", "Логи 100 сообщ"] },
    medium: { name: "Средний", icon: "🟡", price: 249, features: ["Всё из Лёгкого", "Рассылка", "Админ-панель", "Чёрный список", "Статистика"] },
    hard: { name: "Сложный", icon: "🔴", price: 499, features: ["Всё из Среднего", "Авторассылка", "Google Sheets", "Скидки", "Аналитика", "1000+ юзеров"] }
};
...
function renderShop() {
    ...
    for (let [k, l] of Object.entries(LEVELS)) {
        html += `
            <div class="card ${selectedLevel === k ? 'selected' : ''}" onclick="selectLevel('${k}')">
                <div class="card-header">
                    <div class="card-title">${l.icon} ${l.name}</div>
    ...
    html += `<button class="btn btn-primary" ${!selectedLevel ? 'disabled' : ''} onclick="showPayment()">💎 Заказать бота</button>`;
...
function showPayment() {
    if (!tgUser?.id) { tg.showAlert('❌ Откройте магазин через бота'); return; }
    currentStep = 'payment';
    renderPayment();
}
    ...
    document.getElementById('shop-view').innerHTML = `
        <div class="payment-container">
            <div class="payment-header">
                <h2>💳 Оплата заказа</h2>
                <div class="payment-price">${l.price} ₽</div>
            ...
            <div class="requisites">
                <div class="requisite-row">
                    ...
                    <span class="requisite-value">Т-Банк</span>
                </div>
                <div class="card-number" onclick="copyCard()">
                    2200 7021 5075 4195
                </div>
                <div class="copy-hint">📋 Нажмите на номер карты, чтобы скопировать</div>
                <div class="requisite-row">
                    <span class="requisite-label">👤 Получатель</span>
                    <span class="requisite-value">Мухаммад А</span>
                </div>
            </div>

            <div class="upload-area" onclick="document.getElementById('screenshotInput').click()">
                <div class="upload-icon">📸</div>
                <div class="upload-text" id="uploadText">Прикрепить скриншот оплаты</div>
                <div class="upload-hint">PNG, JPG до 5 МБ</div>
                <input type="file" id="screenshotInput" accept="image/*" style="display:none" onchange="handleScreenshot(event)">
                <div id="preview" class="preview" style="display:none"></div>
            </div>

            <div class="action-buttons">
                <button class="btn btn-success" id="confirmBtn" onclick="confirmOrder()" disabled>✅ Я оплатил(а)</button>
                <button class="btn btn-secondary" onclick="goBack()">◀ Назад в магазин</button>
            </div>
        </div>
    `;
}
...
function handleScreenshot(event) {
    ...
    if (file && file.type.startsWith('image/')) {
        ...
        reader.onload = function(e) {
            ...
            document.getElementById('preview').innerHTML = `<img src="${selectedScreenshot}" style="max-width:100%; border-radius:12px">`;
...
async function confirmOrder() {
    ...
    try {
        ...
        if (data.ok) {
            tg.showAlert('✅ Заказ создан! Администратор проверит оплату');
            tg.HapticFeedback?.notificationOccurred('success');
            currentStep = 'select';
...
async function loadProfile() {
    if (!tgUser?.id) { 
        document.getElementById('profile-view').innerHTML = '<div class="empty">❌ Ошибка авторизации</div>'; 
        return; 
    }
    ...
    try {
        ...
        if (orders.length) {
            ordersHtml = orders.map(o => {
                ...
                return `
                    <div class="order-item">
                        <div>
                            <span>${icon} ${name}</span>
                            <div style="font-size:12px; color:#888">${o.price}₽ • ${o.created_at?.slice(0,16)}</div>
                        </div>
            ...
            ordersHtml = '<div class="empty">📭 У вас пока нет заказов</div>';
        }
        document.getElementById('profile-view').innerHTML = `
            <div class="card">
                <div class="profile-header">
                    ...
                    <div style="font-size:12px; color:#888">ID: ${tgUser.id}</div>
                </div>
                <div class="order-list">
                    <h4 style="margin-bottom:15px">📋 История заказов</h4>
                    ${ordersHtml}
                </div>
                ${hasCompleted ? '<button class="btn btn-primary" style="margin-top:20px" onclick="openReviewModal()">✍️ Оставить отзыв</button>' : ''}
            </div>
        `;
    } catch(e) { 
        document.getElementById('profile-view').innerHTML = '<div class="empty">❌ Ошибка загрузки</div>';
    }
}
...
async function loadReviews() {
    ...
    try {
        ...
        if (reviews.length) {
            document.getElementById('reviews-view').innerHTML = `
                <div class="card">
                    ...
                    ${reviews.map(r => `
                        <div class="review-card">
                            ...
                            <div class="review-date">${r.created_at?.slice(0,10)}</div>
                        </div>
                    `).join('')}
                </div>
            `;
        } else {
            document.getElementById('reviews-view').innerHTML = '<div class="card empty">⭐ Пока нет отзывов. Будьте первым!</div>';
        }
    } catch(e) { 
        document.getElementById('reviews-view').innerHTML = '<div class="empty">❌ Ошибка загрузки</div>';
    }
}

function openReviewModal() {
    if (!selectedLevel) { 
        tg.showAlert('❌ Сначала выберите уровень бота в магазине'); 
        return; 
    }
...
async function submitReview() {
    ...
    if (!text) { 
        tg.showAlert('❌ Напишите текст отзыва'); 
        return; 
    }
...
if __name__ == "__main__":
    ...
    print("🔍 Проверка оплаты: сумма + дата + банк/оплата")
    print("🚫 Если фото не похоже на чек оплаты → автоматический отказ")
