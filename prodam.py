...
# ================= SHOP =================
LEVELS = {
    "easy": {"name": "Лёгкий", "price": 99, "icon": "🟢", "color": "#22c55e", "features": ["Защита от спама", "Автоответы", "Приветствие", "Кнопка меню", "Логи 100 сообщ"]},
    "medium": {"name": "Средний", "price": 249, "icon": "🟡", "color": "#eab308", "features": ["Всё из Лёгкого", "Рассылка", "Админ-панель", "Чёрный список", "Статистика"]},
    "hard": {"name": "Сложный", "price": 499, "icon": "🔴", "color": "#ef4444", "features": ["Всё из Среднего", "Авторассылка", "Google Sheets", "Скидки", "Аналитика", "1000+ юзеров"]}
}
...
def add_order(user_id, username, level, screenshot=None):
    ...
    cur.execute("INSERT INTO orders (user_id, username, level, price, screenshot, created_at) VALUES (?, ?, ?, ?, ?, ?)",
...
def start(m):
    ...
    bot.send_message(m.chat.id, "🌟 **BotShop Premium**\n\nНажми на кнопку ниже, чтобы открыть магазин", parse_mode="Markdown", reply_markup=kb)
...
def view_order(call):
    ...
    if order["screenshot"]:
        try:
            ...
            bot.send_photo(call.message.chat.id, BytesIO(img), caption=text, parse_mode="Markdown", reply_markup=order_action_keyboard(order_id, order["status"]))
        except:
            bot.send_message(call.message.chat.id, text + "\n⚠️ Ошибка скриншота", parse_mode="Markdown", reply_markup=order_action_keyboard(order_id, order["status"]))
    else:
        bot.send_message(call.message.chat.id, text, parse_mode="Markdown", reply_markup=order_action_keyboard(order_id, order["status"]))
...
def send_broadcast(message, target, admin_chat_id):
    ...
    if target == "all":
        c = conn()
        users = c.execute("SELECT DISTINCT user_id FROM orders").fetchall()
        c.close()
        count = 0
        for user in users:
            try:
        ...
        c = conn()
        users = c.execute("SELECT DISTINCT user_id FROM orders WHERE status='completed'").fetchall()
        c.close()
        count = 0
        for user in users:
            try:
...
def create_order():
    ...
    bot.send_message(ADMIN_ID, f"🆕 **НОВЫЙ ЗАКАЗ!**\n\n📦 #{order_id}\n👤 @{username or user_id}\n{icon} {level} — {LEVELS[level]['price']}₽", parse_mode="Markdown")
